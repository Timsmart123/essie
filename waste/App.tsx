    ...
    <footer style={{ backgroundColor: '#3A241B', color: '#F6F1E9', paddingBottom: 'calc(env(safe-area-inset-bottom) + 64px)' }} className="md:pb-0">
      <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-4 gap-12">
        <div className="md:col-span-1">
          <img src={ecLogo} alt="Essie Chops" className="h-14 mb-5 brightness-0 invert" />
        ...
        <div>
          ...
          {(['home', 'menu', 'celebrations', 'about', 'gallery', 'contact'] as Page[]).map(p => (
...
function HomePage({ onNav }: { onNav: (p: Page) => void }) {
  ...
  const categories = [
    { title: 'Celebration Cakes', desc: 'Custom tiered cakes for birthdays, weddings, and every milestone worth marking.', img: IMGS.cake1, page: 'menu' as Page },
    { title: 'Surprise Platters', desc: 'Beautifully curated birthday platters — the perfect gift that says everything.', img: IMGS.platter, page: 'celebrations' as Page },
    { title: 'Nigerian Pastries', desc: 'Puff Puff, Meat Pies, Chin Chin — made fresh to authentic family recipes.', img: IMGS.pastry, page: 'menu' as Page },
    { title: 'Catering & Events', desc: 'Full-service catering for corporate events, weddings, and family gatherings.', img: IMGS.catering, page: 'celebrations' as Page },
  ]

  const trustCards = [
    { icon: '🌿', title: 'Freshly Made to Order', desc: 'Every item is prepared fresh when your order is confirmed — never pre-packaged.' },
    { icon: '🍲', title: 'Authentic Nigerian Recipes', desc: 'Traditional family recipes passed down with love and made with the finest ingredients.' },
    { icon: '✨', title: 'Beautiful Presentation', desc: 'Your celebration deserves to look as good as it tastes. Every order is styled to impress.' },
    { icon: '🇬🇧', title: 'Reliable UK Service', desc: 'Based in London, serving across the UK with care, punctuality, and pride.' },
  ]
...
function MenuPage() {
  ...
  const items: { name: string; desc: string; price: string; img: string; cat: MenuCategory }[] = [
  ...
  const cats: MenuCategory[] = ['All', 'Pastries', 'Small Chops', 'Cakes', 'Platters', 'Comfort Food']
...
function AboutPage() {
  const values = [
    { icon: '🤝', title: 'Authenticity', desc: 'We cook from the heart, using time-honoured Nigerian recipes that honour our heritage.' },
    { icon: '🎉', title: 'Joy', desc: 'Food is a celebration in itself. Every product we make is designed to bring people together.' },
    { icon: '🏆', title: 'Craftsmanship', desc: 'Quality is non-negotiable. We take immense pride in every detail, from ingredients to presentation.' },
    { icon: '💛', title: 'Community', desc: "We're part of the communities we serve — sharing culture through food, one celebration at a time." },
    { icon: '🌱', title: 'Integrity', desc: 'We are honest, reliable, and transparent. If we say it will be done, it will be done — and done beautifully.' },
    { icon: '✨', title: 'Excellence', desc: 'Good enough is never our standard. We push for extraordinary in everything we create.' },
  ]
...
function GalleryPage() {
  ...
  const filters: GalleryFilter[] = ['All', 'Cakes', 'Platters', 'Pastries', 'Catering', 'Events']
          ...
          <div className="p-8 rounded-2xl bg-white" style={{ boxShadow: '0 2px 16px rgba(58,36,27,0.06)' }}>
            ...
            <div className="space-y-5 text-sm">
              {[
                ...
                { icon: '📍', label: 'Based in', val: 'London, United Kingdom' },
                { icon: '🚚', label: 'Delivery Areas', val: 'Greater London (nationwide on request)' },
                { icon: '🕐', label: 'Business Hours', val: 'Monday–Saturday, 9am–7pm' },
              ].map(d => (
